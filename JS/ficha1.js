const data = {
    name: 'Nuit',
    player: 'Mayfcb',
    occupation: 'Amnésico / Sensação-especifica',
    age: 21,
    sex: 'female',
  ...
  $('.effortBar').css(
    'width',
    `${calculateBar(data.effort.current, data.effort.max)}%`
  )
  $('#effortCount').text(`${data.effort.current}/${data.effort.max}`)
  $('#effortCurrent').val(data.effort.current)
  $('#effortMax').val(data.effort.max)
  
  const diceModal = $('#diceAttributes')
  const lifeModal = $('#lifeModal')
  const effortModal = $('#effortModal')
  ...
  $(window).click(function (event) {
    ...
    } else if (event.target.id == 'effortModal') {
      effortModal.css('display', 'none')
  ...
  $('.effortBar').click(function () {
    console.log(this)
    effortModal.css('display', 'block')
  })
  ...
  $('#intact').change(function () {
    if (this.checked) {
      console.log('Modo são ativado!')
    } else {
      console.log('Modo são desativado!')
    }
  })
  $('#healthy').change(function () {
    if (this.checked) {
      console.log('Modo saudavel ativado!')
    } else {
      console.log('Modo saudavel desativado!')
    }
  })
  
  $('#affected').change(function () {
    if (this.checked) {
      console.log('Modo afetado ativado!')
    } else {
      console.log('Modo afetado desativado!')
    }
  })
  
  $('#disturbed').change(function () {
    if (this.checked) {
      console.log('Modo pertubado ativado!')
    } else {
      console.log('Modo pertubado desativado!')
    }
  })
  ...
  $('#changeEffort').submit(function (event) {
    let current = Number($('#effortCurrent').val())
    const max = Number($('#effortMax').val())
  
    if (current > max) {
      alert('Os Pontos de Esforço atual não pode ser maior que a maxima!')
      current = max
    }
  
    data.effort.current = current
    data.effort.max = max
    $('.effortBar').css('width', `${calculateBar(current, max)}%`)
    $('#effortCount').text(`${current}/${max}`)
